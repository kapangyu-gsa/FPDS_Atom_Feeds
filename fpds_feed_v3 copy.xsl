...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="entry-nodeset" select="/f:feed/f:entry/f:content"/>
    ...
    <xsl:template match="node()|@*" mode="check-field" >
        ...
        <xsl:if test="$path">
            <!-- <xsl:apply-templates select="$entry-nodeset" mode="do-field"/> -->
         </xsl:if>
        <xsl:if test="not($path)">
            <xsl:apply-templates select="$entry-nodeset" mode="do-field2"/>
        </xsl:if>
    </xsl:template>

    <xsl:template match="f:entry" mode="do-field">
        <!-- <xsl:variable name="thisentry" select="."/> -->
        <xsl:value-of select="$delimiter"/>
        <!-- <xsl:for-each select="./content/ns1:award/ns1:awardID/ns1:awardContractID/ns1:agencyID"> -->
            <!-- <xsl:value-of select="descendant::signedDate"/> -->
        <!-- </xsl:for-each> -->
    ...
    <xsl:template match="f:entry" mode="do-field2">
        <xsl:value-of select="$delimiter"/>
        <xsl:if test="position() = last()">
            <xsl:value-of select="$newline"/>
        </xsl:if>
    ...
    <xsl:template match="/">
        ...
        <!-- awardID -->
        <xsl:apply-templates mode="check-field" >
            <xsl:with-param name="fieldname">agencyID</xsl:with-param>
            <xsl:with-param name="path" select="//ns1:award/ns1:awardID/ns1:awardContractID/ns1:agencyID2"/></xsl:apply-templates>
